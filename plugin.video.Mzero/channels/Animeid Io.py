...
header_string = "|User-Agent=Mozilla/5.0" \
                "&Referer=http://animeid.io&Cookie="
...
def findvideos(item):
    ...
    url_api = scrapertools.find_single_match(data, "http:\/\/api\.animeid\.io\/[^\"]+")
...
def get_cookie_value():
    cookies = os.path.join(config.get_data_path(), 'cookies', 'animeid.io.dat')
    cookiedatafile = open(cookies, 'r')
    cookiedata = cookiedatafile.read()
    cookiedatafile.close()
    cfduid = scrapertools.find_single_match(cookiedata, "animeid.*?__cfduid\s+([A-Za-z0-9\+\=]+)")
    cfduid = "__cfduid=" + cfduid + ";"
    cf_clearance = scrapertools.find_single_match(cookiedata, "animeid.*?cf_clearance\s+([A-Za-z0-9\+\=\-]+)")
