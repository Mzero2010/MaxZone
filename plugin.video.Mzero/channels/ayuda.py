...
# ----------------------------------------------------------------------
# Mzero - XBMC Plugin
# ayuda - Videos de ayuda y tutoriales para Mzero
...
def mainlist(item):
    logger.info("Mzero.channels.ayuda mainlist")
    itemlist = []
    ...
    if not config.is_xbmc():
        ...
        if config.get_setting("realdebridpremium") == "false":
            title = "Activar cuenta real-debrid (Marca la casilla en la ventana de configuración de Mzero para continuar)"
            action = ""
        elif token_auth:
...
def tutoriales(item):
    ...
    for playlist in playlists:
        if playlist.title == "Tutoriales de Mzero":
...
def force_creation_advancedsettings(item):
    ...
    if os.path.exists(advancedsettings_kodi):
        logger.info("Mzero.channels.ayuda La ruta de advanced settings del usuario existe!")

        if platformtools.dialog_yesno("Mzero",
            ...
            if not os.path.exists(fichero_backup):
                ...
                if platformtools.dialog_yesno("Mzero",
                    ...
                    platformtools.dialog_notification("Mzero",
                                                      "Backup terminado!")
                    logger.info("Mzero.channels.ayuda Backup terminado!")
                else:
                    platformtools.dialog_notification("Mzero",
                                                      "Backup no modificado")
                    logger.info("Mzero.channels.ayuda Backup no modificado!")
            ...
            if os.path.exists(advancedsettings_same):
                logger.info("Mzero.channels.ayuda Archivo de comparacion eliminado")
                os.remove(advancedsettings_same)
            if os.path.exists(advancedsettings_trans):
                logger.info("Mzero.channels.ayuda Archivo de translacion eliminado")
    ...
    logger.info("Mzero.channels.ayuda 'force_creation_advancedsettings' method finnished")

    return []


def recover_advancedsettings(item):
    logger.info("Mzero.channels.ayuda recover_advancedsettings")
    ...
    if platformtools.dialog_yesno("Mzero",
        ...
        if os.path.exists(fichero_backup):
            logger.info("Mzero.channels.ayuda Existe un backup de advancedsettings.xml")
            ...
            logger.info("Mzero.channels.ayuda No hay ningun backup disponible")
            if platformtools.dialog_yesno("Mzero",
                ...
                platformtools.dialog_notification("Mzero", "Backup hecho!")
                logger.info("Mzero.channels.ayuda Backup terminado!")
            else:
                platformtools.dialog_notification("Mzero", "Backup no hecho!")
                logger.info("Mzero.channels.ayuda Backup no hecho!")

    else:
        platformtools.dialog_notification("Mzero",
                                          "Operacion cancelada por el usuario")
        logger.info("Mzero.channels.ayuda Optimizacion de adavancedsettings.xml cancelada!")

    return []


def realdebrid(item):
    logger.info("Mzero.channels.ayuda realdebrid")
    itemlist = []
...
def request_access():
    logger.info("Mzero.channels.ayuda request_access")
...
def authentication(item):
    logger.info("Mzero.channels.ayuda authentication")
    import urllib
    from core import channeltools
    from core import jsontools
    from core import scrapertools
