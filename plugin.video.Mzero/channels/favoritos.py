...
def mainlist(item):
    logger.info("Mzero.core.favoritos mainlist")
    itemlist = []
...
def addFavourite(item):
    logger.info("Mzero.core.favoritos addFavourite")
    #logger.debug(item.tostring('\n'))
...
def delFavourite(item):
    logger.info("Mzero.core.favoritos delFavourite")
    #logger.debug(item.tostring('\n'))
...
def renameFavourite(item):
    logger.info("Mzero.core.favoritos renameFavourite")
    #logger.debug(item.tostring('\n'))
...
def check_bookmark(readpath):
    ...
    if itemlist:
        ...
        for item in itemlist:
            data = "ActivateWindow(10025,&quot;plugin://plugin.video.Mzero/?" + item.tourl() + "&quot;,return)"
