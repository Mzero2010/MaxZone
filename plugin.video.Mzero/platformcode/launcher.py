...
def start():
    ...
    """
    logger.info("Mzero.platformcode.launcher start")

    # Test if all the required directories are created
    config.verify_directories_created()

def run():
    logger.info("Mzero.platformcode.launcher run")
    ...
    logger.info("Mzero.platformcode.launcher "+item.tostring())
    ...
    try:
        ...
        if item.action == "":
            logger.info("Mzero.platformcode.launcher Item sin accion")
            return
        ...
        if ( item.action=="selectchannel" ):
            ...
            if config.get_setting("updatecheck2") == "true":
                logger.info("Mzero.platformcode.launcher Check for plugin updates enabled")
                ...
                logger.info("Mzero.platformcode.launcher Check for plugin updates disabled")
        ...
        # Action in certain channel specified in "action" and "channel" parameters
            ...
            logger.info("Mzero.platformcode.launcher channel_file=%s" % channel_file)
            ...
            logger.info("Mzero.platformcode.launcher running channel "+channel.__name__+" "+channel.__file__)

            # Special play action
            if item.action == "play":
                logger.info("Mzero.platformcode.launcher play")

                # First checks if channel has a "play" function
                if hasattr(channel, 'play'):
                    logger.info("Mzero.platformcode.launcher executing channel 'play' method")
                ...
                # If player don't have a "play" function, not uses the standard play from xbmctools
                    ...
                    logger.info("Mzero.platformcode.launcher executing core 'play' method")
                    xbmctools.play_video(item)
            ...
            # Special action for findvideos, where the plugin looks for known urls
                ...
                # If not, uses the generic findvideos function
                    ...
                    logger.info("Mzero.platformcode.launcher no channel 'findvideos' method, "
            ...
            # Special action for searching, first asks for the words then call the "search" function
                ...
                logger.info("Mzero.platformcode.launcher search")
            ...
            # For all other actions
                ...
                logger.info("Mzero.platformcode.launcher executing channel '"+item.action+"' method")
                itemlist = getattr(channel, item.action)(item)
        ...
        import traceback
        logger.error("Mzero.platformcode.launcher "+traceback.format_exc())
        ...
        if hasattr(e, 'reason'):
            logger.info("Mzero.platformcode.launcher Razon del error, codigo: "+str(e.reason[0])+", Razon: "+str(e.reason[1]))
            texto = config.get_localized_string(30050) # "No se puede conectar con el sitio web"
            ok = ventana_error.ok ("plugin", texto)
        
        # Grab server response errors
        elif hasattr(e,'code'):
            logger.info("Mzero.platformcode.launcher codigo de error HTTP : %d" %e.code)
        ...
        if canal:
            xbmcgui.Dialog().ok(
                ...
                "porque hay algo mal en Mzero.\nPara saber más detalles, consulta el log.")
        else:
            xbmcgui.Dialog().ok(
                "Se ha producido un error en Mzero",
                "Comprueba el log para ver mas detalles del error." )

def set_server_list():
    logger.info("Mzero.platformcode.launcher.set_server_list")
...
def filtered_servers(itemlist, server_white_list, server_black_list):
    logger.info("Mzero.platformcode.launcher.filtered_servers")
    new_list = []
    white_counter = 0
    black_counter = 0

    logger.info("Mzero.platformcode.launcher filtered_servers whiteList %s" % server_white_list)
    logger.info("Mzero.platformcode.launcher filtered_servers blackList %s" % server_black_list)

    if len(server_white_list) > 0:
        logger.info("Mzero.platformcode.launcher filtered_servers whiteList")
    ...
    if len(server_black_list) > 0:
        logger.info("Mzero.platformcode.launcher filtered_servers blackList")
        for item in itemlist:
            logger.info("item.title " + item.title)
    ...
    logger.info("Mzero.platformcode.launcher filtered_servers whiteList server %s has #%d rows" %
                (server_white_list, white_counter))
    logger.info("Mzero.platformcode.launcher filtered_servers blackList server %s has #%d rows" %
...
def play_from_library(item, channel, server_white_list, server_black_list):
    logger.info("Mzero.platformcode.launcher play_from_library")

    logger.info("Mzero.platformcode.launcher play_from_library item.server=#"+item.server+"#")
    ...
    try:
        ...
        item = elegido
    logger.info("Mzero.platformcode.launcher play_from_library Elegido %s (sub %s)" % (item.title,
                                                                                               item.subtitle))
