...
def play_video(item, strm=False):
    logger.info("Mzero.platformcode.platformtools play_video")
    ...
    # Si no puedes ver el vídeo te informa
        ...
        if item.server != "":
            ...
            dialog_ok("No puedes ver ese vídeo porque...", "El servidor donde está alojado no está",
                      "soportado en Mzero todavía", item.url)
...
def get_video_seleccionado(item, seleccion, video_urls):
    ...
    logger.info("Mzero.platformcode.platformstools mediaurl=" + mediaurl)
    if mediaurl == "":
...
def set_player(item, xlistitem, mediaurl, view, strm):
    ...
    # Si es un fichero strm no hace falta el play
        ...
        if config.get_setting("player_mode") == "3" or "megacrypter.com" in mediaurl:
            ...
            logger.info("Mzero.platformcode.platformstools playersettings=" + playersettings)

            if config.get_system_platform() == "xbox":
                player_type = xbmc.PLAYER_CORE_AUTO
                if playersettings == "0":
                    player_type = xbmc.PLAYER_CORE_AUTO
                    logger.info("Mzero.platformcode.platformstools PLAYER_CORE_AUTO")
                elif playersettings == "1":
                    player_type = xbmc.PLAYER_CORE_MPLAYER
                    logger.info("Mzero.platformcode.platformstools PLAYER_CORE_MPLAYER")
                elif playersettings == "2":
                    player_type = xbmc.PLAYER_CORE_DVDPLAYER
                    logger.info("Mzero.platformcode.platformstools PLAYER_CORE_DVDPLAYER")
