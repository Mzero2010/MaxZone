...
def add_new_folder(item, totalItems=0):
    #logger.info('Mzero.platformcode.xbmctools add_new_folder item='+item.tostring())
    ...
    #logger.info("Mzero.platformcode.xbmctools add_new_folder itemurl="+itemurl)
...
def add_new_video(item, IsPlayable='false', totalItems = 0):
    #logger.info('Mzero.platformcode.xbmctools add_new_video item='+item.tostring())
    ...
    itemurl = '%s?%s' % ( sys.argv[ 0 ] , item.tourl())
    #logger.info("Mzero.platformcode.xbmctools add_new_video itemurl="+itemurl)
...
def play_video(item,desdefavoritos=False,desdedescargados=False,desderrordescargas=False,strmfile=False):
    ...
    logger.info("Mzero.platformcode.xbmctools play_video")
    ...
    # Si no puedes ver el vídeo te informa
        ...
        if item.server!="":
            ...
            resultado = advertencia.ok( "No puedes ver ese vídeo porque...","El servidor donde está alojado no está","soportado en Mzero todavía",item.url)
    ...
    logger.info("Mzero.platformcode.xbmctools mediaurl="+mediaurl)
    if mediaurl=="":
        if server == "unknown":
    ...
    #Movido del conector "torrent" aqui
        ...
        if config.get_setting("player_mode")=="3" or "megacrypter.com" in mediaurl:
            ...
            logger.info("Mzero.platformcode.xbmctools playersettings="+playersettings)

            if config.get_system_platform()=="xbox":
                player_type = xbmc.PLAYER_CORE_AUTO
                if playersettings == "0":
                    player_type = xbmc.PLAYER_CORE_AUTO
                    logger.info("Mzero.platformcode.xbmctools PLAYER_CORE_AUTO")
                elif playersettings == "1":
                    player_type = xbmc.PLAYER_CORE_MPLAYER
                    logger.info("Mzero.platformcode.xbmctools PLAYER_CORE_MPLAYER")
                elif playersettings == "2":
                    player_type = xbmc.PLAYER_CORE_DVDPLAYER
                    logger.info("Mzero.platformcode.xbmctools PLAYER_CORE_DVDPLAYER")
            ...
            if item.channel=="cuevana" and item.subtitle!="":
                ...
                if item.subtitle!="" and (opciones[seleccion].startswith("Ver") or opciones[seleccion].startswith("Watch")):
                    logger.info("Mzero.platformcode.xbmctools Con subtitulos")
                    setSubtitles()
...
def getLibraryInfo (mediaurl):
    ...
    '''
    if DEBUG:
        logger.info('Mzero.platformcode.xbmctools playlist OBTENCIÓN DE DATOS DE BIBLIOTECA')
...
def playstrm(params,url,category):
    ...
    '''
    logger.info("Mzero.platformcode.xbmctools playstrm url="+url)
    ...
    play_video("Biblioteca Mzero",server,url,category,title,thumbnail,plot,strmfile=True,Serie=serie,subtitle=subtitle)

def renderItems(itemlist, parentitem, isPlayable='false'):
    logger.info("Mzero.platformcode.xbmctools renderItems")
...
def wait2second():
    logger.info("Mzero.platformcode.xbmctools wait2second")
    import time
    contador = 0
    while xbmc.Player().isPlayingVideo()==False:
        logger.info("Mzero.platformcode.xbmctools setSubtitles: Waiting 2 seconds for video to start before setting subtitles")
        time.sleep(2)
        contador = contador + 1
...
def setSubtitles():
    logger.info("Mzero.platformcode.xbmctools setSubtitles")
    import time
    contador = 0
    while xbmc.Player().isPlayingVideo()==False:
        logger.info("Mzero.platformcode.xbmctools setSubtitles: Waiting 2 seconds for video to start before setting subtitles")
    ...
    logger.info("Mzero.platformcode.xbmctools setting subtitle file %s" % subtitlefile)
    xbmc.Player().setSubtitles(subtitlefile)

def trailer(item):
    logger.info("Mzero.platformcode.xbmctools trailer")
    config.set_setting("subtitulo", "false")
...
def alert_no_puedes_ver_video(server,url,motivo):
    if server!="":
        ...
        resultado = advertencia.ok( "No puedes ver ese vídeo porque...","El servidor donde está alojado no está","soportado en Mzero todavía",url)
