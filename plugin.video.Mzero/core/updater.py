...
def checkforupdates():
    ...
    logger.info("Mzero.core.updater checkforupdates version remota="+str(numero_version_publicada))

    # Lee la versión local
    numero_version_local = get_current_plugin_version()
    logger.info("Mzero.core.updater checkforupdates version local="+str(numero_version_local))

    hayqueactualizar = numero_version_publicada > numero_version_local
    logger.info("Mzero.core.updater checkforupdates -> hayqueactualizar="+repr(hayqueactualizar))
...
def download_and_install(remote_file_name,local_file_name):
    logger.info("Mzero.core.updater download_and_install from "+remote_file_name+" to "+local_file_name)
    ...
    logger.info("Mzero.core.updater installation_target=%s" % installation_target)
...
def update_channel(channel_name):
    logger.info("Mzero.core.updater update_channel "+channel_name)
    ...
    try:
        ...
        logger.info("Mzero.core.updater update_channel remote_data="+data)
        remote_version = int( scrapertools.find_single_match(data,'<version>([^<]+)</version>') )
    except:
        remote_version = 0

    logger.info("Mzero.core.updater update_channel remote_version=%d" % remote_version)
    ...
    if updated:
        logger.info("Mzero.core.updater update_channel downloading...")
