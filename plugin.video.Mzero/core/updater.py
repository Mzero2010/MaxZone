...
#Todas las plataformas son "Mzero-nombre-plataforma-version.zip" excepto para xbmc que es "Mzero-xbmc-plugin-version.zip"
...
def checkforupdates():
    logger.info("Mzero.core.updater checkforupdates")

    # Descarga el fichero con la versión en la web
    logger.info("Mzero.core.updater Verificando actualizaciones...")
    logger.info("Mzero.core.updater Version remota: "+REMOTE_VERSION_FILE)
    ...
    logger.info("Mzero.core.updater version remota="+tag_publicada+" "+version_publicada)
    
    # Lee el fichero con la versión instalada
    logger.info("Mzero.core.updater fichero local version: "+LOCAL_VERSION_FILE)
    ...
    logger.info("Mzero.core.updater version local="+tag_local+" "+version_local)
    ...
    if version_publicada is None or version_local is None:
        logger.info("Mzero.core.updater comprobando el tag")
        from distutils.version import StrictVersion
        hayqueactualizar = StrictVersion(tag_publicada) > StrictVersion(tag_local)
        
    else:
        logger.info("Mzero.core.updater comprobando la version")
...
def update(item):
    # Descarga el ZIP
    logger.info("Mzero.core.updater update")
    remotefilename = REMOTE_FILE+item.version+".zip"
    localfilename = LOCAL_FILE+item.version+".zip"
    if os.path.exists(localfilename):
      os.remove(localfilename)
    logger.info("Mzero.core.updater remotefilename=%s" % remotefilename)
    logger.info("Mzero.core.updater localfilename=%s" % localfilename)
    logger.info("Mzero.core.updater descarga fichero...")
    inicio = time.clock()
    ...
    fin = time.clock()
    logger.info("Mzero.core.updater Descargado en %d segundos " % (fin-inicio+1))
    
    # Lo descomprime
    logger.info("Mzero.core.updater descomprime fichero...")
    import ziptools
    unzipper = ziptools.ziptools()
    destpathname = DESTINATION_FOLDER
    logger.info("Mzero.core.updater destpathname=%s" % destpathname)
    unzipper.extract(localfilename,destpathname)
    
    # Borra el zip descargado
    logger.info("Mzero.core.updater borra fichero...")
    os.remove(localfilename)
    logger.info("Mzero.core.updater ...fichero borrado")

def get_channel_remote_url(channel_name):

    _remote_channel_url_ = "https://raw.githubusercontent.com/tvalacarta/Mzero/master/python/main-classic/"
    ...
    logger.info("Mzero.core.updater remote_channel_url="+remote_channel_url)
    logger.info("Mzero.core.updater remote_version_url="+remote_version_url)
...
def get_channel_local_path(channel_name):
    ...
    logger.info("Mzero.core.updater local_channel_path="+local_channel_path)
    logger.info("Mzero.core.updater local_version_path="+local_version_path)
    logger.info("Mzero.core.updater local_compiled_path="+local_compiled_path)
    
    return local_channel_path , local_version_path , local_compiled_path

def updatechannel(channel_name):
    logger.info("Mzero.core.updater updatechannel('"+channel_name+"')")
    ...
    try:
        ...
        logger.info("Mzero.core.updater remote_data="+data)
    ...
    logger.info("Mzero.core.updater remote_version=%d" % remote_version)
    ...
    if os.path.exists( local_version_path ):
        ...
        data = infile.read()
        infile.close();
        logger.info("Mzero.core.updater local_data="+data)
        ...
        local_version = 0
    logger.info("Mzero.core.updater local_version=%d" % local_version)
    
    # Comprueba si ha cambiado
    updated = remote_version > local_version

    if updated:
        logger.info("Mzero.core.updater updated")
        download_channel(channel_name)

    return updated

def download_channel(channel_name):
    logger.info("Mzero.core.updater download_channel('"+channel_name+"')")
    # Canal remoto
    remote_channel_url , remote_version_url = get_channel_remote_url(channel_name)
    ...
    try:
        ...
        outfile.flush()
        outfile.close()
        logger.info("Mzero.core.updater Grabado a " + local_channel_path)
    except:
        logger.info("Mzero.core.updater Error al grabar " + local_channel_path)
        import sys
    ...
    try:
        ...
        outfile.flush()
        outfile.close()
        logger.info("Mzero.core.updater Grabado a " + local_version_path)
    except:
        import sys
        for line in sys.exc_info():
            logger.error( "%s" % line )
