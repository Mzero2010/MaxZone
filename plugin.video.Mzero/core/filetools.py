...
def read(path):
    ...
    if path.lower().startswith("smb://"):
        ...
        try:
            ...
            logger.info("Mzero.core.filetools read: ERROR al leer el archivo: {0}".format(path))
...
def write(path, data):
    ...
    if path.lower().startswith("smb://"):
        ...
        try:
            ...
            logger.info("Mzero.core.filetools write: Error al guardar el archivo: {0}".format(path))
            return False
        else:
            return True
...
def exists(path):
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            logger.info("Mzero.core.filetools exists: No es posible conectar con la ruta")
...
def mkdir(path):
    ...
    @type path: str
    """
    logger.info("Mzero.core.filetools mkdir "+path)
    ...
    if path.lower().startswith("smb://"):
        try:
            ...
            import traceback
            logger.info("Mzero.core.filetools mkdir: Error al crear la ruta "+traceback.format_exc())
...
def remove_tags(title):
    ...
    """
    logger.info("Mzero.core.filetools remove_tags")
