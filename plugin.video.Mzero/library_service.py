...
def create_tvshows_from_xml():
    logger.info("Mzero.platformcode.library_service create_tvshows_from_xml")
...
def create_tvshows_from_json(_actualizado):
    logger.info("Mzero.platformcode.library_service create_tvshows_from_json")
...
def main():
    logger.info("Mzero.library_service Actualizando series...")
    p_dialog = None
    ...
    try:
        ...
        if config.get_setting("updatelibrary") == "true":
            ...
            for i, tvshow_file in enumerate(show_list):
                ...
                logger.info("Mzero.library_service serie="+serie.contentSerieName)
                logger.info("Mzero.library_service Actualizando " + path)
                logger.info("Mzero.library_service url " + serie.url)
                ...
                if serie.active:
                    ...
                    try:
                        ...
                        logger.info("Mzero.library_service Cargando canal: " + pathchannels + " " +
                                    serie.channel)
                        ...
                        try:
                            ...
                            logger.info("Mzero.library_service Error al guardar los capitulos de la serie")
            ...
            logger.info("No actualiza la biblioteca, está desactivado en la configuración de Mzero")

    except Exception as ex:
        logger.info("Mzero.library_service Se ha producido un error al actualizar las series")
        template = "An exception of type {0} occured. Arguments:\n{1!r}"
        message = template.format(type(ex).__name__, ex.args)
        logger.info(message)
